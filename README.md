...
The app is live at https://www.toolzpointt.com/. See `docs/VERCEL_DEPLOYMENT.md` for redeployment and verification. All 78 tools are marked `reviewed: true` and indexing is enabled (`NEXT_PUBLIC_ALLOW_INDEXING=true`) as of 2026-09-19. In the Vercel project's **Production** environment variables only, set `NEXT_PUBLIC_SITE_URL=https://www.toolzpointt.com` and `NEXT_PUBLIC_ALLOW_INDEXING=true` — keep both unset/`false` on preview deployments so previews stay on their own preview URL and never get indexed. A tool added later starts with `reviewed: false`; flip it to `true` once its content has been checked, since the sitemap and each tool page's robots meta tag both key off that flag.
